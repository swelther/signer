...
class Signer
  ...
  def signature_node
    @signature_node ||= begin
      ...
      unless @signature_node
        @signature_node = Nokogiri::XML::Node.new('ds:Signature', document)
        @signature_node.add_namespace_definition("ds", 'http://www.w3.org/2000/09/xmldsig#')
  ...
  def signed_info_node
    ...
    unless node
      node = Nokogiri::XML::Node.new('ds:SignedInfo', document)
      node.add_namespace_definition("ds", 'http://www.w3.org/2000/09/xmldsig#')
      signature_node.add_child(node)
      canonicalization_method_node = Nokogiri::XML::Node.new('ds:CanonicalizationMethod', document)
  ...
  def binary_security_token_node
    ...
    unless node
      node = Nokogiri::XML::Node.new('ds:BinarySecurityToken', document)
      ...
      reference_node = Nokogiri::XML::Node.new("ds:#{wsse_ns}:Reference", document)
  ...
  def x509_data_node
    ...
    issuer_number_node.content = cert.serial.to_i
...
#    issuer_serial_node.add_child(issuer_number_node)
    ...
    cetificate_node    = Nokogiri::XML::Node.new('ds:X509Certificate', document)
...
#    data_node.add_child(issuer_serial_node)
  ...
  def digest!(target_node, options = {})
    ...
    reference_node = Nokogiri::XML::Node.new('ds:Reference', document)
    reference_node['URI'] = id.to_s.size > 0 ? "##{id}" : ""
    signed_info_node.add_child(reference_node)

    transforms_node = Nokogiri::XML::Node.new('ds:Transforms', document)
    reference_node.add_child(transforms_node)

    transform_node = Nokogiri::XML::Node.new('ds:Transform', document)
    ...
    digest_method_node = Nokogiri::XML::Node.new('ds:DigestMethod', document)
    digest_method_node['Algorithm'] = @digester.digest_id
    reference_node.add_child(digest_method_node)

    digest_value_node = Nokogiri::XML::Node.new('ds:DigestValue', document)
  ...
  def sign!(options = {})
    ...
    signature_value_node = Nokogiri::XML::Node.new('ds:SignatureValue', document)
